...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_tag_on_update(self):
        ...
        # we're going to check that if we update a recipe, but we provide
        # a different tag that doesn't exist, we're going to create that tag
        # in the system. If you change the to one that doesn't exist. We're
        # going to automatically create that.
        recipe = create_recipe(user=self.user)

        payload = {'tags': [{'name': 'Lunch'}]}
        # Then we have the URL. So we generated the URL for the detailed
        # endpoint for the recipe that we previously created.
        url = detail_url(recipe.id)
        # So we're doing a http patch, which is a partial update request,
        # which means we only want to update the values which we provide
        # in the payload.
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        # And then the new tag we retrieving the new tag that we expect
        # to be created by doing tag two objects.
        new_tag = Tag.objects.get(user=self.user, name='Lunch')
        # Finally, we're checking that the new tag that we retrieved
        # exists in the recipe.
        self.assertIn(new_tag, recipe.tags.all())
        # we shouldn't need to do refresh DB here.
    ...
    def test_update_recipe_assign_tag(self):
        ...
        tag_lonch = Tag.objects.create(user=self.user, name='Lunch')
        payload = {'tags': [{'name': 'Lunch'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(tag_lonch, recipe.tags.all())
        self.assertNotIn(tag_breakfast, recipe.tags.all())

    def test_clear_recipe_tags(self):
        """Test clearing all a recipes tags."""
        tag = Tag.objects.create(user=self.user, name='Dessert')
        recipe = create_recipe(user=self.user)  # sample recipe
        recipe.tags.add(tag)
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        payload = {'ingredients': [{'name': 'Chili'}]}  # new ingredient
        url = detail_url(recipe.id)
    ...
    def test_clear_recipe_ingredients(self):
        """Test clearing all recipes ingredients."""
    ...
    def tst_filter_by_tags(self):
        """Test filtering recipes by tags."""
        r1 = create_recipe(user=self.user, title='Thai Vegetable Curry')
        r2 = create_recipe(user=self.user, title='Aubergine with Rahini')
    ...
    def test_filter_by_ingredients(self):
        ...
        r3 = create_recipe(user=self.user, title='Red Lentl Dahl')
